...
const schemaRole = {
    'properties': {
        'RoleCode': {'type': ['string', 'number'], 'pattern': '^[a-zA-Z0-9\\(\\)\\-&.\\\\,/_ ]+$'},
        'RoleDesc': {'type': ['string', 'number'], 'pattern': '^[a-zA-Z0-9\\(\\)\\-&.\\\\,/_ ]+$', 'maxLength': 2000}
    },
    'required': [
        'RoleCode',
    ],
    'errorMessage': {
        'properties': {
            'RoleCode': 'Role Code should be alphanumeric',
            'RoleDesc': 'Role Description should be alphanumeric with max characters 2000'
        },
        'required': {
            'RoleCode': 'Role Code is required'
        }
    }
};
...
export class RoleUploadDataService {
    ...
    constructor(private http: HttpClient, private global: Global, private rest: RestService) {
        this.url = this.global.getapiendpoint() + 'role/BulkUpload';
        this.getAllRole();
    }

    getAllRole() {
        this.roles = [];
        this.rest.getAll(this.global.getapiendpoint() + "role/GetAllRoleList").subscribe((data: any) => {
        ...
        files.forEach(file => {
            ...
            reader.onload = (e: any) => {
                ...
                this.data.forEach((row, index) => {
                    ...
                    const objDuplicate = this.data.filter(en => en.RoleCode.toString().toLowerCase().trim() == row.RoleCode.toString().toLowerCase().trim());
                    if (objDuplicate.length > 1) {
                        this.data[index].Error += ' | Duplicate records';
                        this.errorFlag = true;
                    }

                    const objRole = this.roles.find(en => en.RoleCode.toLowerCase() == row.RoleCode.toString().toLowerCase().trim());
                    if (objRole) {
                        this.data[index].Error += ' | Role already exist';
                        this.errorFlag = true;
                    }
